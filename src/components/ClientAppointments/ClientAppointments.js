...
export const ClientAppointments = () => {
    ...
    useEffect(() => {
        fetch(`${API}/employees?_expand=user`)
            .then((res) => res.json())
            .then((data) => {
    ...
    useEffect(() => {
        ...
        fetch(
            `${API}/appointments?client_Id=${parkerUserObject.id}&_expand=pet&_expand=service&sort=date&_order=desc`
        )
            .then((res) => res.json())
            .then((data) => {
    ...
    const apptlist = appointments.map((appt, i) => {
        const employeeUser = employeeUsers.find(
            (el) => el.userId === appt.employee_Id
        )
        return employeeUser ? (
            <tr key={`appts--${i}`}>
                ...
                <td>
                    <Link
                        className={styles.apptLink}
                        to={`/petform/${appt.pet.id}`}
                    >
                        {appt.pet.name}
                    </Link>
                </td>
                <td>
                    <Link
                        className={styles.apptLink}
                        to={`/client/employeeview/${employeeUser.id}`}
                    >
                ...
                <td>
                    <button
                        type="button"
                        className={`btn btn-sm btn-outline-light ${styles.deleteButton}`}
                        onClick={() => {
            ...
            <div className="container">
                ...
                {appointments.length > 0 ? (
                    <table className={`table ${styles.apptTable}`}>
                        ...
                        <tbody>{apptlist}</tbody>
                    </table>
                ) : (
                    <p>You currently do not have any appointments</p>
                )}
                <button
                    className={`btn btn-outline-light ${styles.apptButton}`}
                    onClick={() => {

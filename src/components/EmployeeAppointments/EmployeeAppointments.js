...
export const EmployeeAppointments = () => {
    ...
    useEffect(() => {
        fetch(`${API}/clients?_expand=user`)
            .then((res) => res.json())
            .then((data) => {
                setClientUsers(data)
            })
    ...
    useEffect(() => {
        ...
        fetch(
            `${API}/appointments?employee_Id=${parkerUserObject.id}&_expand=pet&_expand=service&sort=date&_order=desc`
        )
            .then((res) => res.json())
            .then((data) => {
            ...
            <tr key={`appts--${i}`}>
                ...
                <td>
                    <Link
                        ...
                        to={`/employee/petview/${appt.pet.id}`}
                    >
                        {appt.pet.name}
                    </Link>
                </td>
                ...
                <td>
                    <Link
                        className={styles.apptLink}
                        to={`/employee/clientview/${appt.client_Id}`}
                    >
                        View Client Details
                    </Link>
                </td>
